...
def load_recording(recording_file, header=None,key='val'):
    rootname, extension = os.path.splitext(recording_file)
    header_file = rootname + '.hea'

    # ==========================================================
    # ======================== FIX =============================
    # ==========================================================
    # STEP 1: Add a "pre-flight check" for the header file.
    # We do this BEFORE calling any wfdb functions that might crash.
    try:
        with open(header_file, 'r') as f:
            header_lines = f.readlines()
        
        # Filter for non-empty, non-comment lines. A valid header MUST have at least one.
        valid_lines = [line for line in header_lines if line.strip() and not line.strip().startswith('#')]
        
        if not valid_lines:
            # If the list is empty, the header is invalid.
            print(f"WARNING: Skipping corrupt or empty header file: {header_file}")
            return None # Return None to signal that this file should be skipped.
    except FileNotFoundError:
        print(f"WARNING: Header file not found for {recording_file}. Skipping.")
        return None
    # --- End of Pre-flight Check ---


    # STEP 2: Wrap the original code in a try-except block for ultimate safety.
    try:
        if extension == '.dat':
            # This call is now protected by our check above.
            recording = wfdb.rdrecord(rootname)
            return recording.p_signal
        if extension == '.mat':
            recording = loadmat(recording_file)[key]
        ...
        # This will catch any other unexpected errors from wfdb or loadmat
...
def get_leads(header):
    leads = list()
    num_leads = 0  # Initialize num_leads
    for i, l in enumerate(header.split('\n')):
        ...
        if i == 0:
            ...
            # ==========================================================
            # Add a defensive check to ensure the line is not malformed
            if len(entries) < 2:
                # You can print a warning if you want to see which files are bad
                # print(f"WARNING: Malformed first line in header. Could not determine number of leads. Line: '{l}'")
                # Break the loop entirely if the first line is bad, as we can't proceed.
                break 
            
            try:
                num_leads = int(entries[1])
            except (ValueError, IndexError):
                # print(f"WARNING: Could not parse number of leads from header line: '{l}'")
                break # Cannot proceed without num_leads
            # ==========================================================
            # === END OF FIX ===========================================
            # ==========================================================
...
def get_adc_gains(header, leads):
    ...
    for i, l in enumerate(header.split('\n')):
        ...
        entries = l.split(' ')
        if i == 0:
             # Add a defensive check here as well for consistency
            if len(entries) < 2:
                break
            try:
                num_leads = int(entries[1])
