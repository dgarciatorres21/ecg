...
# --- CONFIGURATION ---
DATA_DIR = 'C:/Users/dgarc/Desktop/Dissertation/ecg-image-kit/codes/ecg-image-generator/outputData/Generated_data'
YOLO_LABELS_DIR = 'C:/Users/dgarc/Desktop/Dissertation/ecg-image-kit/data/yolo_labels'

# --- MODIFIED PART 1: ADD THE NEW CLASS ---
CLASS_MAP = {
    'I': 0
    , 'II': 1
    , 'III': 2
...
# This is a threshold in pixels. If a Lead II box is wider than this,
# we'll classify it as the rhythm strip.
# You can adjust this value if needed, but 1000 should be very safe.
...
def process_json_annotations():
    ...
    print(f"Found {len(json_files)} .json files. Starting conversion to YOLO format...")
    ...
    for json_filename in tqdm(json_files, desc="Converting JSON to YOLO"):
        ...
        for lead_info in data['leads']:
            if 'lead_bounding_box' in lead_info and 'lead_name' in lead_info:
                ...
                # First, get the bounding box in [x_min, y_min, width, height] format
                xywh_bbox = convert_corners_to_xywh(corners)

                # --- MODIFIED PART 2: THE LOGIC TO DISTINGUISH LEADS ---
                class_id = -1 # Default to an invalid ID
                
                # If the lead is 'II', we need to check if it's the long rhythm strip
                if lead_name == 'II':
                    box_width = xywh_bbox[2] # The width is the 3rd element
                    if box_width > rhythm_WIDTH_THRESHOLD:
                        class_id = CLASS_MAP['L']
                    else:
                        class_id = CLASS_MAP['II']
                # For all other leads, the mapping is simple
                elif lead_name in CLASS_MAP:
                    class_id = CLASS_MAP[lead_name]
                
                # If we successfully found a class_id, process it
                if class_id != -1:
