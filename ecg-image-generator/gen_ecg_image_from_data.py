...
def get_parser():
    ...
    parser.add_argument('--generate_masks', action='store_true', default=False)
    parser.add_argument('--mask-output-dir', type=str, help='Specific directory to save generated masks.')
    return parser
...
def run_single_file(args):
        ...
        fonts_folder_path = os.path.join(script_dir, 'Fonts')
        font_files = os.listdir(fonts_folder_path)
        random_font_file = random.choice(font_files)
        font = os.path.join(fonts_folder_path, random_font_file)
        ...
        out_array = get_paper_ecg(
            ...
            standard_colours=standard_colours, full_mode=args.full_mode,
            bbox=args.lead_bbox, columns=args.num_columns, seed=args.seed,
            generate_masks=args.generate_masks,
            mask_output_directory=args.mask_output_dir
        )
        ...
        for out in out_array:
            ...
            if args.store_config:
                ...
                json_path = rec_tail + '.json'
                if os.path.exists(json_path) and os.path.getsize(json_path) > 0:
                    try:
                        with open(json_path, 'r') as file:
                            json_dict = json.load(file)
                    except json.JSONDecodeError:
                        print(f"  - Warning: Could not decode JSON for {os.path.basename(out)}. Starting with empty config.")
                        json_dict = {}
                else:
                    print(f"  - Warning: JSON file for {os.path.basename(out)} is missing or empty. Starting fresh.")
                    json_dict = {}
            ...
            #     x_offset = 0
            #     y_offset = 0
            if args.store_config == 2 and json_dict is not None:
