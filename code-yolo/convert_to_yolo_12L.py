...
def process_json_annotations(data_dir, yolo_labels_dir):
    ...
    print(f"Found {len(json_files)} .json files in '{data_dir}'. Starting conversion for 12-lead format...")
    
    for json_filename in tqdm(json_files, desc="Converting JSON to YOLO (12-lead)"):
    ...
    print("\nConversion complete!")
    print(f"YOLO label files (12-lead format) have been saved to: {yolo_labels_dir}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Convert JSON annotations to YOLOv8 format (12-lead version).")
